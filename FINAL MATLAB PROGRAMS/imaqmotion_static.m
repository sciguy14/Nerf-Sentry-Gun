function imaqmotion_static(obj)
...
try
    ...
    set(obj, 'Tag', appTitle, 'FramesAcquiredFcnCount', 1, ...
        'TimerFcn', @localFrameCallbackStatic, 'TimerPeriod', 0.1);
...
%-------------------------------------
function localFrameCallbackStatic(vid, event)
...
localUpdateFigStatic(vid, appdata.figureHandles, frame, background);
appdata.background = frame;
set(vid, 'UserData', appdata);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function localUpdateFigStatic(vid, figData, frame, background)
...
if (x > 1) && (log == 1) && (dlmread('data/auth.txt') == 1) && (trows > 35)
...
if numTurns == 4
    if (x > 1)
        dlmwrite('data/gunpos.txt',uint32(x));
    end
    dlmwrite('data/turn.txt',0);
else
