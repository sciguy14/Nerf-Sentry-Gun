...
figure = gcf;
set(figure, 'KeyPressFcn', {@myFunctionPress, chan});
set(figure, 'KeyReleaseFcn', {@myFunctionRelease, chan});
...
%     guidata(src, handles); 
    control(chan,1,1);
    set(controls(2),'String','Firing: On');
end
...
%     guidata(src, handles); 
    control(chan,3,1);
    set(controls(3),'String','Right: On');
end
end
...
%     guidata(src, handles); 
    control(chan,1,0);
    set(controls(2),'String','Firing: Off');
end
if strcmp(k,'rightarrow')
%     axes(handles.axes1);
%     handles.cats(2)=0;
...
%     guidata(src, handles);
    control(chan,2,0);
    set(controls(4),'String','Left: Off');
end
if strcmp(k,'leftarrow')
%     axes(handles.axes1);
%     handles.cats(3)=0;
...
%     guidata(src, handles);
    control(chan,3,0);
    set(controls(3),'String','Right: Off');
end
end
